...
__host__ __device__ float chingada_II(float lgm){
	float coeff[12] = { 0.0932136220831,0.18297695053,-0.0278161516718,
                     -0.0143101501618,-0.00421497860572,0.00445822721607,
                     -0.000379055530599,-0.00236945869922,0.00334384619699,
                     -0.000917014382652,-0.00094528825527,0.000847800540058};
	float rango[2] = {12.232903400213145,15.335207281597203};
	float p = 0.0;
	for(int i = 0; i < 12; i++){
		p += coeff[i]*cos((i+1)*M_PI*(lgm-rango[0])/(rango[1]-rango[0]))*(i+1)*M_PI/(rango[1]-rango[0]);
	}
	p = 1./(rango[1] - rango[0]) - p;
  return(p);
}
